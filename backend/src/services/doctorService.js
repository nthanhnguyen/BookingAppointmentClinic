...
let saveDetailInforDoctor = (inputData) => {
    return new Promise(async (resolve, reject) => {
        try {
            let checkObjs = checkRequireFields(inputData);
            if (checkObjs.isValid === false) {
                resolve({
                    errCode: 1,
                    errMessage: `Missing parameter: ${checkObjs.element}`
                })
            } else {
                ...
                if (inputData.action == 'CREATE') {
                    ...
                    if (doctorMarkdown) {
                        ...
                        //doctorMarkdown.updateAt = new Date(); // Ko có này cx đc
